...
const REQUEST_HEADER_NAMES = new Map([
  ...
  ["accept-language", "Accept-Language"], ["sec-fetch-site", "Sec-Fetch-Site"],
  ["sec-fetch-mode", "Sec-Fetch-Mode"], ["sec-fetch-dest", "Sec-Fetch-Dest"],
  ["sec-ch-ua", "Sec-CH-UA"], ["sec-ch-ua-mobile", "Sec-CH-UA-Mobile"],
  ["sec-ch-ua-platform", "Sec-CH-UA-Platform"], ["cache-control", "Cache-Control"], ["pragma", "Pragma"]
]);

function cleanPath(url) { return String(url || "").toLowerCase().split("?")[0].split("#")[0]; }
...
function looksLikeDouyinVideo(url) {
  ...
  return host.endsWith("douyinvod.com") || host.includes("douyin-vod") || host.includes("bytevcloud") ||
    lower.includes("/aweme/v1/play") || lower.includes("video_id=") || lower.includes("mime_type=video") ||
    lower.includes("/video/tos/") || lower.includes("/tos-cn-") || lower.includes("playwm");
}
...
function looksLikeAd(url) { const lower = String(url || "").toLowerCase(); return AD_MARKERS.some((marker) => lower.includes(marker)); }
function headerValue(headers = [], name) { const target = String(name || "").toLowerCase(); return String(headers.find((h) => String(h.name || "").toLowerCase() === target)?.value || ""); }
...
function contentLengthFromHeaders(headers = []) { const value = Number(headerValue(headers, "content-length") || 0); return Number.isFinite(value) ? value : 0; }
function isMediaContentType(value) { const type = String(value || "").toLowerCase(); return type.startsWith("video/") || type.startsWith("audio/") || type.includes("mpegurl") || type.includes("dash+xml"); }
...
function scoreMedia(item) {
  ...
  if (looksLikeDouyinVideo(item.url)) score += 25;
  if (item.likelyAd) score -= 100;
  if (item.type === "segment") score -= 1000;
  return score + streamPriority(item);
}

function getTabItems(tabId) { if (!MEDIA_BY_TAB.has(tabId)) MEDIA_BY_TAB.set(tabId, new Map()); return MEDIA_BY_TAB.get(tabId); }
function newState(pageUrl = "") { return { playbackConfirmed: false, playbackAt: 0, pageUrl, playbackWidth: 0, playbackHeight: 0, playbackDuration: 0 }; }
function getTabState(tabId) { if (!TAB_STATE.has(tabId)) TAB_STATE.set(tabId, newState()); return TAB_STATE.get(tabId); }
function resetTab(tabId, pageUrl = "") { MEDIA_BY_TAB.delete(tabId); TAB_STATE.set(tabId, newState(pageUrl)); }
function visibleItems(tabId) {
  ...
  const all = [...(MEDIA_BY_TAB.get(tabId)?.values() || [])]
    ...
    .map((item) => ({ ...item, playbackWidth: state.playbackWidth || 0, playbackHeight: state.playbackHeight || 0, playbackDuration: state.playbackDuration || 0 }));
...
async function refreshBadge(tabId) {
  ...
  const state = getTabState(tabId); const items = visibleItems(tabId);
...
function addCandidate(tabId, candidate) {
  ...
  const items = getTabItems(tabId); const existing = items.get(candidate.url);
  ...
  const merged = {
    url: candidate.url, type, contentType,
    ...
    likelyAd: looksLikeAd(candidate.url), detectedAt: Date.now()
  };
  merged.score = scoreMedia(merged); items.set(candidate.url, merged);
  if (items.size > 260) {
    const keep = [...items.values()].sort((a, b) => b.score - a.score || b.detectedAt - a.detectedAt).slice(0, 160);
    items.clear(); for (const item of keep) items.set(item.url, item);
  }
  refreshBadge(tabId);
}
...
chrome.webRequest.onHeadersReceived.addListener((details) => {
  ...
  const headers = details.responseHeaders || []; const contentType = contentTypeFromHeaders(headers);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === "OMNIFETCH_MEDIA_CANDIDATES") {
    const tabId = sender.tab?.id; for (const candidate of message.items || []) addCandidate(tabId, candidate); sendResponse({ ok: true }); return;
  }
  if (message?.type === "OMNIFETCH_PLAYBACK_STARTED") {
    const tabId = sender.tab?.id;
    if (Number.isInteger(tabId)) {
      const state = getTabState(tabId); state.playbackConfirmed = true; state.playbackAt = Date.now();
      state.pageUrl = message.pageUrl || state.pageUrl || "";
      state.playbackWidth = Number(message.videoWidth || state.playbackWidth || 0);
      state.playbackHeight = Number(message.videoHeight || state.playbackHeight || 0);
      state.playbackDuration = Number(message.duration || state.playbackDuration || 0);
      refreshBadge(tabId);
    }
    sendResponse({ ok: true }); return;
  }
  if (message?.type === "OMNIFETCH_RESET_TAB_CAPTURE") {
    const tabId = sender.tab?.id; if (Number.isInteger(tabId)) { resetTab(tabId, message.pageUrl || ""); refreshBadge(tabId); } sendResponse({ ok: true }); return;
  }
  if (message?.type === "OMNIFETCH_GET_MEDIA") {
    const state = getTabState(message.tabId);
    sendResponse({ ok: true, playbackConfirmed: Boolean(state.playbackConfirmed), playbackWidth: state.playbackWidth || 0, playbackHeight: state.playbackHeight || 0, playbackDuration: state.playbackDuration || 0, items: visibleItems(message.tabId).slice(0, 80) });
    return;
  }
  if (message?.type === "OMNIFETCH_GET_REQUEST_CONTEXT") { sendResponse({ ok: true, headers: findRequestContext(message.url, message.tabId) }); return; }
  if (message?.type === "OMNIFETCH_CLEAR_MEDIA") { resetTab(message.tabId); refreshBadge(message.tabId); sendResponse({ ok: true }); return; }
  if (message?.type === "OMNIFETCH_DIRECT_DOWNLOAD") {
    chrome.downloads.download({ url: message.url, saveAs: true }, (downloadId) => { const error = chrome.runtime.lastError?.message; sendResponse({ ok: !error, downloadId, error }); });
    return true;
  }
});

chrome.tabs.onRemoved.addListener((tabId) => { MEDIA_BY_TAB.delete(tabId); TAB_STATE.delete(tabId); });
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => { if (changeInfo.status === "loading" && changeInfo.url) { resetTab(tabId, changeInfo.url); refreshBadge(tabId); } });
